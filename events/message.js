...
module.exports = message => {
  ...
  message.react('❌');
  return message.reply(`<:duyuru:822791227879063562> Botumuz Şu Anda Bakımda. Yaklaşık **${RESULT}** Önce Bakıma Alınmış.`);
  }
  };
  };
    if(cmd) {
  if(message.author.id !== ayarlar.sahip) {
  if(db.fetch(message.guild.owner.user.id) && !db.fetch(message.author.id)) return message.channel.send('<:duyuru:822791227879063562> Bu sunucunun sahibi botun karalistesinde. O yüzden sende komut kullanamazsın.'); 
  if(db.fetch(message.author.id)) return message.channel.send('<:duyuru:822791227879063562> Sen botun karalistesindesin. Komutları kullanamazsın.');
  };
  };
  if (cmd) {
